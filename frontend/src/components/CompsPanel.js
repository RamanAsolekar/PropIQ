    ...
    <div style={{ padding: '0 0 8px' }}>
      ...
      {compStats && compStats.count > 0 && (
        <div style={{ background: '#EEEDFE', border: '1px solid #CECBF6',
          ...
          <div style={{ fontSize: 11, fontWeight: 600, color: '#3C3489',
            textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
            {locality} Market — {compStats.count} listings analysed
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {[
              { label: 'Avg ask', value: `₹${compStats.avg_price_sqft?.toLocaleString()}/sqft` },
              { label: 'Low', value: `₹${compStats.min_price_sqft?.toLocaleString()}/sqft` },
              { label: 'High', value: `₹${compStats.max_price_sqft?.toLocaleString()}/sqft` },
            ].map(m => (
              <div key={m.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#2C2C2A' }}>{m.value}</div>
      ...
      <div style={{ fontSize: 11, fontWeight: 600, color: '#888780',
        textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8, paddingLeft: 2 }}>
        {comps.length} Comparable Properties
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {comps.map((comp, i) => {
          const sc = SIMILARITY_COLORS[comp.similarity_label] || SIMILARITY_COLORS["Comparable"];
          return (
            <div key={i} style={{ background: '#fff', border: '1px solid rgba(44,44,42,0.10)',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 4 }}>
                {[
                  { label: 'Size', value: `${comp.size_sqft?.toLocaleString()} sqft` },
                  { label: 'Age', value: `${comp.age_years?.toFixed(0)} yrs` },
                  { label: '₹/sqft', value: `₹${comp.price_per_sqft?.toLocaleString()}` },
                  { label: 'Total', value: formatINR(comp.total_price) },
                ].map(d => (
                  <div key={d.label} style={{ background: '#F7F6F2', borderRadius: 6,
                    padding: '5px 7px', textAlign: 'center' }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: '#2C2C2A' }}>{d.value}</div>
                    <div style={{ fontSize: 9, color: '#888780', marginTop: 1 }}>{d.label}</div>
                  </div>
                ))}
              </div>
            </div>
