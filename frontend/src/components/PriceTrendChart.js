...
export default function PriceTrendChart({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`${BASE_URL}/api/v1/trends/${encodeURIComponent(locality)}`, {
  ...
  const changeColor =
    change > 0 ? "#0F6E56" : change < 0 ? "#A32D2D" : "#888780";
      ...
      {summary && (
        ...
        >
          {[
            {
              label: "Start",
              value: `₹${summary.start_price?.toLocaleString()}/sqft`,
            },
            {
              label: "Current",
              value: `₹${summary.current_price?.toLocaleString()}/sqft`,
            },
            {
              label: "Last 6m",
              value: `${summary.last_6m_change_pct > 0 ? "+" : ""}${summary.last_6m_change_pct}%`,
            },
            {
              label: "Annualised",
              value: `${summary.annualized_return_pct}%/yr`,
            },
          ].map((p) => (
            <div
              key={p.label}
