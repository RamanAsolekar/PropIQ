...
export default function LTVCalculator({ ltvAnalysis, marketValueMid }) {
  ...
  const currentLTV = loanAmount
    ? Math.round((loanAmount / marketValueMid) * 100)
    : recommended_ltv_pct;
  ...
  const zoneColors = {
    green: {
      ...
      text: "#085041",
      border: "#9FE1CB",
      label: "Within RBI limits",
    },
    amber: {
      bg: "#FAEEDA",
      text: "#633806",
      border: "#FAC775",
      label: "Marginal — review needed",
    },
    red: {
      bg: "#FCEBEB",
      text: "#791F1F",
      border: "#F09595",
      label: "Exceeds recommended LTV",
    },
  };

  const currentZone =
    currentLTV > rbi_max_ltv_pct
      ? "red"
      : currentLTV > pfl_internal_ltv_pct
        ? "amber"
        : "green";
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <span>
            <span style={{ color: "#BA7517", fontWeight: 700 }}>|</span> PFL (
            {pfl_internal_ltv_pct}%)
          </span>
          <span>
            <span style={{ color: "#A32D2D", fontWeight: 700 }}>|</span> RBI max
            ({rbi_max_ltv_pct}%)
          </span>
          <span
            style={{
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input
            type="range"
            min={minLoan}
            max={Math.round(marketValueMid * 0.95)}
            step={100000}
            value={loanAmount || maxLoan}
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {[
            {
              label: `PFL (${pfl_internal_ltv_pct}%)`,
              pct: pfl_internal_ltv_pct,
            },
            ...
            <button
              key={b.label}
              onClick={() =>
                setLoanAmount(Math.round((marketValueMid * b.pct) / 100))
              }
              style={{
                flex: 1,
                padding: "6px 4px",
