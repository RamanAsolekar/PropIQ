...
def _severity_color(severity: str) -> colors.Color:
    return {"high": FLAG_RED, "medium": FLAG_AMBER, "low": MID_GRAY}.get(
        severity, MID_GRAY
    )
...
def generate_pdf_report(assessment: dict) -> bytes:
    ...
    header_data = [
        [
            Paragraph(
                "<b>PropIQ</b>",
                ParagraphStyle("logo", parent=h1, fontSize=22, textColor=BRAND_PURPLE),
            ),
            Paragraph(
    ...
    story.append(
        HRFlowable(width="100%", thickness=1.5, color=BRAND_PURPLE, spaceAfter=4 * mm)
    )
    ...
    prop_rows = [
        [
            "Locality",
            assessment.get("locality", "—"),
            "Zone Tier",
            enrichment.get("zone_tier", "—").title(),
        ],
        [
            "Property Type",
            assessment.get("prop_type", "—").replace("_", " ").title(),
            "Size",
            f"{assessment.get('size_sqft',0):,.0f} sqft",
        ],
        [
            "Circle Rate",
            f"₹{enrichment.get('circle_rate_per_sqft',0):,}/sqft",
            "Infra Score",
            f"{enrichment.get('infra_score',0):.0f}/100",
        ],
        [
            "Listing Density",
            f"{enrichment.get('listing_density',0):.0%}",
            "Model MAPE",
            f"{assessment.get('model_mape_pct','—')}%",
        ],
    ]
    ...
    val_data = [
        ...
        [
            ...
            Paragraph(
                f"<b>{_fmt_inr(mv_mid)}</b>",
                ParagraphStyle("vb", parent=body, textColor=BRAND_TEAL, fontSize=11),
            ),
        ...
        [
            "Distress Sale Value",
            Paragraph(
                f"<b>{_fmt_inr(int((dv[0]+dv[1])/2))}</b>",
                ParagraphStyle("vb2", parent=body, textColor=FLAG_AMBER, fontSize=11),
            ),
            f"{_fmt_inr(dv[0])}  –  {_fmt_inr(dv[1])}",
        ],
        [
            "Price per Sqft",
            f"₹{assessment.get('price_per_sqft_estimate',0):,}/sqft",
            "P50 estimate",
        ],
        [
            ...
            Paragraph(
                f"<b>{rpi:.1f}/100</b>",
                ParagraphStyle("rpi", parent=body, textColor=rpi_color, fontSize=11),
            ),
            rpi_label,
        ],
        ...
        [
            ...
            Paragraph(
                ...
                ParagraphStyle(
                    "conf",
                    parent=body,
                    textColor=FLAG_GREEN if conf >= 0.8 else FLAG_AMBER,
                    fontSize=11,
                ),
            ),
        ...
        for d in drivers[:6]:
            ...
            driver_rows.append(
                [
                    feat_name,
                    Paragraph(
                        f"<b><font color='{color_str}'>{sign} {_fmt_inr(abs(impact))}</font></b>",
    ...
    if cv and cv.get("image_analyzed"):
        ...
        # Use hardcoded hex strings — ReportLab Color objects have no .hexval() method
        if adj_pct > 0:
            adj_hex = "#3B6D11"
        elif adj_pct < 0:
            adj_hex = "#A32D2D"
        ...
        cv_rows = [
            ...
            [
                ...
                Paragraph(
                    f"<b><font color='{adj_hex}'>{adj_str}</font></b>",
    ...
    # ── LTV ANALYSIS (Gap 7/8) ─────────────────────────────────────────────
    ltv = assessment.get("ltv_analysis")
    if ltv:
        story.append(Paragraph("Loan-to-Value (LTV) & Funding Recommendation", h2))
        ltv_rows = [
            ["Metric", "Value", "Policy Rationale"],
            [
                "Recommended LTV",
                f"{ltv.get('recommended_ltv_pct',0):.1f}%",
                ltv.get("pfl_rationale", "—"),
            ],
            [
                "Max Loan Amount",
                _fmt_inr(ltv.get("max_loan_amount", 0)),
                "On Market Value",
            ],
            [
                "Max Loan (Distress)",
                _fmt_inr(ltv.get("max_loan_on_distress_value", 0)),
                "On Distress Value",
            ],
            [
                "RBI LTV Cap",
                f"{ltv.get('rbi_max_ltv_pct',0):.1f}%",
                ltv.get("rbi_rationale", "—"),
            ],
        ]
        ltvt = Table(ltv_rows, colWidths=["25%", "25%", "50%"])
        ltvt.setStyle(
            TableStyle(
                [
    ...
    # ── COMPARABLE SALES (Gap 2) ──────────────────────────────────────────
    comps = assessment.get("comps", [])
    if comps:
        story.append(Paragraph("Comparable Market Evidence", h2))
        comp_rows = [["MATCH", "LOCALITY", "SIZE", "AGE", "PRICE", "SIMILARITY"]]
        for c in comps[:4]:
            comp_rows.append(
                [
                    c.get("prop_type", "").replace("_", " ").title(),
                    c.get("locality", "—"),
                    f"{c.get('size_sqft',0):,.0f}",
                    f"{c.get('age_years',0):.0f}y",
                    _fmt_inr(c.get("total_price", 0)),
                    c.get("similarity_label", "—"),
                ]
            )
        ct = Table(comp_rows, colWidths=["18%", "22%", "15%", "10%", "20%", "15%"])
        ct.setStyle(
            TableStyle(
                [
    ...
    memo = assessment.get("credit_memo")
    if memo:
        story.append(Paragraph("AI-Generated Credit Memo Summary", h2))
        story.append(
            Paragraph(f"<b>Executive Summary:</b> {memo.get('summary','—')}", body)
        )
        story.append(Spacer(1, 2 * mm))
        story.append(
            Paragraph(
                f"<b>Risk Assessment:</b> {memo.get('risk_assessment','—')}", body
            )
        )
        story.append(Spacer(1, 2 * mm))
        story.append(
            Paragraph(f"<b>Recommendation:</b> {memo.get('recommendation','—')}", body)
        )
        story.append(Spacer(1, 4 * mm))

    # ── DISCLAIMER ─────────────────────────────────────────────────────────
    story.append(
        HRFlowable(width="100%", thickness=0.5, color=MID_GRAY, spaceAfter=3 * mm)
    )
    disclaimer = (
...
if __name__ == "__main__":
    ...
    sample = {
        ...
        "key_drivers": [
            ...
            {
                "feature": "is_standard_config",
                "impact_inr": 900000,
                "direction": "positive",
            },
            {
                "feature": "listing_density",
                "impact_inr": 400000,
                "direction": "positive",
            },
        ],
        "risk_flags": [
            {
                "flag": "moderate_building_age",
                "severity": "low",
                "detail": "Building is 8 years old — minor depreciation applied.",
            }
        ],
        "enrichment": {
            "zone_tier": "prime",
